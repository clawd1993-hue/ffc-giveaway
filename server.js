...
<link rel="stylesheet" href="/static/style.css?v=3">${extraHead}</head><body>
...
function landingPage({ prefillEmail = '', ref = '', ended = false }) {
  const coachOpts = COACHES.map(c => `<option value="${esc(c)}">${esc(c)}</option>`).join('');
  ...
  <section class="rules">
    ...
    <ul>
      ...
      <li>Multiple $1,000 implementation scholarships toward the Elite program.</li>
